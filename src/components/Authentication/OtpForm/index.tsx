...
import { otpUser } from "@/lib/Features/Auth/otpslice";
...
const OtpVerificationForm: React.FC = () => {
        ...
        onSubmit: async (values, { resetForm, setSubmitting, setFieldError }) => {
            ...
            try {
                const resultAction = await dispatch(
                    otpUser({ otp: otpString})
                );
                if (otpUser.fulfilled.match(resultAction)) {
                    ...
                    resetForm();
                } else if (otpUser.rejected.match(resultAction)) {
    ...
    //         const resultAction = await dispatch(forgetUser({ email: storedEmail }));

    //         if (forgetUser.fulfilled.match(resultAction)) {
    //             const message = resultAction.payload?.message;
    //             toast.success(message);
    //         } else if (forgetUser.rejected.match(resultAction)) {
    //             const errorMessage = resultAction.payload as string;
    //             toast.error(errorMessage);
    //         }
    //     } catch (error) {
