...
const AddNewYachts: React.FC = () => {
    ...
    const handleRemoveImage = (index: number) => {
        const files = Array.isArray(formik.values["Gallery Images"]) ? [...formik.values["Gallery Images"]] : [];
    ...
    const formik = useFormik<FormYachtsValues>({
        ...
        onSubmit: async (values, { setSubmitting }) => {
            try {
                ...
                const resultAction = await dispatch(addYachts({
        ...
        <>
            <form onSubmit={formik.handleSubmit}>
                                ...
                                {section.fields.map((field, index) => {
                                    ...
                                    return (
                                        <div key={index} className={`${isFileUpload
                                            ...
                                            {isDropdown ? (
                                                <>
                                                    <div className={`bg-[#F0F2F4] rounded-lg px-3 py-2 w-full ${fieldError ? "border border-[#DB2828]" : ""}`}>
                                                        <select
                                                            name={fieldName}
                                                ...
                                                <>
                                                    <div className={`text-[#222222] w-full bg-[#F0F2F4] rounded-lg px-3 py-2  ${fieldError ? "border border-[#DB2828]" : ""}`}>
                ...
                <div className="mt-6 flex justify-end">
                    <button type="submit" className="rounded-full px-[16px] py-[8px] bg-[#001B48] hover:bg-[#222222] text-white flex items-center justify-center cursor-pointer font-medium">
                        Save and Continue
                    </button>
                </div>
            </form>
